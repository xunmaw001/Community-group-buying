    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="退货理由" prop="tuihuoliyou">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.tuihuoliyou" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.tuihuoliyou" label="退货理由" prop="tuihuoliyou">
                    <span v-html="ruleForm.tuihuoliyou"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.tuihuoliyou = this.ruleForm.tuihuoliyou.replace(reg,'../../../ssm641l4/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tuihuotuikuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tuihuotuikuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
